...
from .fnspkg_field import FNspkgDensityField, FNspkgField


@dataclass
class FNspkgModelConfig(ModelConfig):
    # field(default_factory=lambda: ...可以确保每个实例都有自己的独立副本，避免了多个实例共享同一个可变对象的问题。
    _target: Type = field(default_factory=lambda: FNspkgModel)
    near_plane: float = 1.0  # 2.0
    """How far along the ray to start sampling."""
    far_plane: float = 100.0  # 6.0
...
class FNspkgModel(Model):

    config: FNspkgModelConfig

    def populate_modules(self):
        """初始化 fields & modules."""
        super().populate_modules()  # 获取当前类的父类（超类）的引用
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        # 比起Field文件中的同名函数, 更专注于计算单个采样点的属性
